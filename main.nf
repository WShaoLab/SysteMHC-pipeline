...
if(params.help) {
    ...
    log.info "*  --MHClass:       of which MHC class the analysis is to perform, MHC1 or MHC2. (default: $params.MHClass)"
    ...
    log.info "*  --instrument:       The resolution of MS2 spectra, High or Low.   (default: $params.instrument)"
    log.info "*  --protein_db:    fasta formatted sequence database file (default: $params.protein_db)"
    log.info "*  --neo:       is the fasta formated sequence file a customized one? Yes or no. (default: $params.neo)"
    log.info ""
    log.info "Comet Results will be stored in Results/Comet"
    log.info "MSGF+ Results will be stored in Results/MSGF"
    log.info "Msfragger Results will be stored in Results/Fragger"
    log.info "FDR calculation Results will be stored in Results/Peplevel_FDR"
    log.info "Sample Library will be stored in Results/SampleLib"
    log.info "Sample PTMs Results will be stored in Results/SamplePTMs"
    log.info "Sample non-canonical peptides will be stored in Results/SampleVariant"
    log.info "MHC specific library will be stored in Results/MHCSpecificLib"
    log.info "MHC specific non-canonical peptides will be stored in Results/Variant"
    log.info "MHC specific motifs will be stored in Results/MHCmotifs"
    log.info "MHC libraries will be stored in Results/MHCLib"
    log.info "MHC PTMs will be stored in Results/MHCPTMs"
    log.info "A brief summary will be stored in Results/Summary"
    exit 1
}
